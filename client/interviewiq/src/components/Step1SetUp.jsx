...
function Step1SetUp({onStart}) {
  ...
  const handleUploadResume = async (e) => {
    ...
    try {
      ...
      const serverMessage = error?.response?.data?.message || error?.message || 'Resume upload failed'
      console.error('Resume upload failed:', serverMessage)
      alert(serverMessage)
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const serverMessage = error?.response?.data?.message || error?.message || 'Could not start interview'
      console.error('Start interview failed:', serverMessage)
      alert(serverMessage)
      setLoading(false)
    }
  }
    ...
    className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 px-4">
      <div className="w-full max-w-6xl bg-white rounded-3xl shadow-2xl grid md:grid-cols-2 overflow-hidden ">
        ...
        className="p-12 bg-white">
          <h2 className="text-3xl font-bold text-gray-800 mb-8">Interview</h2>
         <div className="space-y-6">
          <div className="relative">
            <FaUserTie className="absolute top-4 left-4 text-gray-400" />
            <input type="text" placeholder="Enter role"
            className="w-full pl-22 pr-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition"
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Experience (ex: 2 years)"
            className="w-full pl-22 pr-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition"
          ...
          {!analysisDone && (
            ...
            onClick={()=>document.getElementById("resumeUpload").click()} className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer hover:border-green-500 hover:bg-green-50 transition">
            ...
            className="bg-gray-50 border border-gray-200 rounded-xl p-5">
              <div className="flex justify-between items-center mb-3">
                ...
                <button
                  ...
                  className="text-xs text-green-600 hover:text-green-700 underline font-medium cursor-pointer"
                >
                  Change Resume
                </button>
              </div>
